...
> En réalité, non, il y a très peu de types fondamentaux, et les principaux sont :
> - `string` : toute chaîne de caractères, peu importe sa longueur.
> - `number` : tout nombre entier ou décimal entre `-9_007_199_254_740_991` et `9_007_199_254_740_991`.
> - `bigint` : tout entier de taille arbitraire.
> - `boolean` : `true` ou `false`.
> - `undefined` : représente une valeur absente.
> - `function` : toute fonction synchrone ou asynchrone, génératrice ou non, ainsi que les constructeurs de classe.
> - `object` : tout objet, tableau et instance de classe.
> - `symbol` : représente les symboles.
>
> Dans d'autres langages, les nombres et les `string` ont beaucoup de déclinaisons différentes. On sépare même l'entier du nombre à virgule.
> Ce qui paraît être une grande quantité de types n'est en réalité que de la **composition de types**.
> Une fonction, ses paramètres, ce qu'elle retourne, un objet, ses propriétés, un tableau et ce qu'il contient : tout cela n'est que de la **composition de types** pour en créer un autre.

---
layout: two-cols-header
---

# Quelles possibilités de typage en TypeScript ?

::left::
```ts
<!--@include: ./examples/typePossibility.ts{1,20} -->
```
::right::
```ts
<!--@include: ./examples/typePossibility.ts{21,40} -->
```
<style>
.two-cols-header {
  column-gap: 10px;
...
> Il est possible d'implémenter du typage :
> - sur des arguments de fonction
> - sur le retour de fonction
> - sur une déclaration de variable
> - avec le mot-clé `satisfies` pour créer un **contrat indirect**.
> - via l'inférence du retour d'une fonction.
> - sur une classe, avec une interface qui impose l'implémentation de propriétés.
> - via une classe abstraite, qui peut jouer un rôle similaire à une interface implémentée.
> - via des prédicats afin d'influencer le type après l'utilisation d'une fonction.
...
Dans le cas où la **valeur utilisée** est égale à `0`, cette fonction nous renverra `null`. Sinon, elle renverra le **type** dont nous avons besoin. On peut dire ici que la fonction `numberIsNotZero` joue un rôle de **constructeur** pour notre **type**.
...
- **Lisibilité** : La déclaration des fonctions est tout de suite plus explicite et compréhensible. Le **diviseur** est explicitement autre chose que `0`.
- **Sécurité** : L'exécution est désormais sécurisée grâce au **typage**. Impossible d'utiliser `0`, donc impossible d'obtenir un résultat erroné à l'exécution.
- **Élégance** : Le **code de vérification** est factorisé. Cette **contrainte** pourrait être utilisée par d'autres fonctions, ce qui éviterait d'avoir une **condition répétée** et un retour à `null` sur chacune d'entre elles à cause d'un paramètre qui serait égal à zéro.
- **Gain de temps** : Moins de **tests unitaires**. Chaque fonction qui implémenterait cette vérification devrait être accompagnée de tests unitaires validant que les **paramètres fournis** ne créent pas de **résultats problématiques**. Il serait pourtant beaucoup plus pertinent de se concentrer sur des tests qui s'occupent uniquement du **métier**, et non de problèmes liés à des **limitations techniques**.
- **Garantie** : Ce genre de **contrat explicite** permet de déléguer sereinement des tâches à des **agents IA** pour la partie d'implémentation.
...
> # Les importations depuis `@duplojs/utils`
> Comme vous avez pu le voir dans l'exemple précédent, la librairie exporte des variables comme `C`, `E` et ...
> Cela correspond à des namespaces appartenant à un domaine :
> - `A` Domaine Array
> - `C` Domaine Clean
> - `E` Domaine Either
> - `DP` Domaine DataParser, avec sa variante `DPE` pour obtenir la version étendue.
> - ...
>
> Seul le domaine `Common` exporte ses fonctions directement.
>
> Il y a plus d'une dizaine de domaines. Je vous propose d'aller voir la doc si vous voulez plus d'informations : https://utils.duplojs.dev
...
- Les `DataParsers` : jouent le rôle de **vérificateurs** lors du runtime.
- Les `Primitives` : servent de **structures de base** pour accueillir des contraintes.
- Les `Constraints` : définissent sur des primitives des **contraintes identifiables** par le typage et également exécutables au runtime.
- Les `NewTypes` : donnent une **identité métier** identifiable et offrent la possibilité d'implémenter des `Constraints`.
...
> # Pourquoi les `DataParsers` ont besoin des `Checkers` ?
> C'est vrai que les `DataParsers` pourraient implémenter directement les contraintes `min`, `max`, `regex`, etc.
> Cependant, ce système d'affinement a été externalisé, car si, en tant que développeur, vous avez besoin d'une vérification très spécifique, il est beaucoup plus facile de créer votre propre `Checker`. Si ce système avait été internalisé, il aurait été impossible d'implémenter votre propre vérification.
>
> Il y a aussi une réponse un peu plus technique. Si les `Checkers` étaient internalisés, à chaque fois que vous importeriez un `DataParser`, tout le code des différentes vérifications serait également importé, alors que vous n'utilisez peut-être qu'une ou deux de ces vérifications. Cela rend le code plus **tree-shaking friendly**.
...
Pour créer le **contrat**, il suffit de déclarer un **type** et d'appeler l'interface `GetConstraint`. Donc, si vous souhaitez indiquer qu'un argument de fonction est un numéro de téléphone, vous utiliserez le type `PhoneNumber` déclaré ici.
...
- Une `Primitive`, c'est juste un objet qui **stocke une valeur inaccessible**.
- Une `Constraint`, c'est juste un **marqueur stocké** sur l'objet d'une `Primitive`.
- Un `NewType`, c'est juste une `Primitive` avec un **nom métier stocké** dessus, et peut-être des `Constraints`.

Et les `DataParsers` s'assurent, pendant la phase d'exécution du code, que les valeurs utilisées pour créer des `Constraints` ou des `NewTypes` correspondent bien à ce qui a été défini.
